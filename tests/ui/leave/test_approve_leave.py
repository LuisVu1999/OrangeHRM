...
def test_approve_leave(page):
    ...
    #1.6.Approve request
    date_in_record = "2025-18-11 (09:00 - 13:00) Half Day"
    leave_type = "CAN - Vacation"
    leave_balance_left = "79.50"
    number_day = "0.50"
    status = "Pending Approval (0.50)"
    ...
    #4. Login by Admin and approve request
    login_page.login(user_name_prepare, password)
    leave_page.navigate_to_leave()
    leave_page.approve_leave(full_name, date_in_record, leave_type,leave_balance_left, number_day, status)
